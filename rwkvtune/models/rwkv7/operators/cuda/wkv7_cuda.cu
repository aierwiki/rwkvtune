...
__global__ void forward_kernel(int T, int H, F_ w_, F_ q_, F_ k_, F_ v_, F_ a_, F_ b_, bf* y_, float* s_, float* sa_) {
    ...
    for (int t = 0; t < T; t++) {
        ...
        __syncthreads();
        q[i] = to_float(q_[ind]);
        // Fix: w computation consistent with official code
        // Official code: w = -F.softplus(-(self.w0 + torch.tanh(xw @ self.w1) @ self.w2)) - 0.5
        // Here w_ is in exp(-exp(w)) form
        // Note: the w_ received here is already preprocessed, see caller for details
